...
- 2af61f4: Pin the published dependency contract (exact `@opencode-ai/plugin` and `open` versions, drop unused `@opencode-ai/sdk` and `zod`) and make the packed TUI build rewrite host-runtime imports in an import-aware way.
- b58c450: Fix `/supabase` failing to load on recent OpenCode versions: the published package shipped raw TSX through `./tui`, and once the host stopped transforming TSX under `node_modules`, the import crashed on `react/jsx-dev-runtime` before the plugin could register.
  ...
  Ship a self-contained declaration for `./tui` so TypeScript consumers can resolve the package without installing host-owned optional peers or enabling `skipLibCheck`.
